create or replace procedure batch_word_to_phrase_translation_vote_reset(
  in p_word_to_phrase_translation_ids bigint[],
  in p_token varchar(512),
  inout p_word_to_phrase_translation_vote_ids bigint[],
...
begin
  ...
  p_word_to_phrase_translation_vote_ids := array[]::bigint[];
  p_error_types := array[]::varchar(32)[];

  for i in 1..v_word_to_phrase_translations_ids_length loop
    v_temp_word_to_phrase_translation_vote_id := 0;
    v_temp_error_type := 'NoError';

    call word_to_phrase_translation_vote_reset(
    ...
    p_word_to_phrase_translation_vote_ids := array_append(
      p_word_to_phrase_translation_vote_ids, 
      v_temp_word_to_phrase_translation_vote_id
    );

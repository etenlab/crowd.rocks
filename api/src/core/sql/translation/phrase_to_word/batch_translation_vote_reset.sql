create or replace procedure batch_phrase_to_word_translation_vote_reset(
  in p_phrase_to_word_translation_ids bigint[],
  in p_token varchar(512),
  inout p_phrase_to_word_translation_vote_ids bigint[],
...
begin
  ...
  p_phrase_to_word_translation_vote_ids := array[]::bigint[];
  p_error_types := array[]::varchar(32)[];

  for i in 1..v_phrase_to_word_translations_ids_length loop
    v_temp_phrase_to_word_translation_vote_id := 0;
    v_temp_error_type := 'NoError';

    call phrase_to_word_translation_vote_reset(
    ...
    p_phrase_to_word_translation_vote_ids := array_append(
      p_phrase_to_word_translation_vote_ids, 
      v_temp_phrase_to_word_translation_vote_id
    );
